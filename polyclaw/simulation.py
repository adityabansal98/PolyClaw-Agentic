...
class AgentArenaSimulation:
    """Virtual exchange for multi-agent paper betting."""
    ...
    def _init_db(self) -> None:
        if self._use_supabase:
            # Supabase schema is expected to be provisioned via SQL migrations.
            return
    ...
    def record_bet(self, *, agent_name: str, recommendation: dict[str, Any], side: str, stake: float) -> None:
        ...
        if self._use_supabase:
            ...
            self._supabase.insert(
                TICKER_TABLE,
                {"event_type": "BET", "message": message, "created_at": now},
            )
            return
    ...
    def settle_open_bets(self, market_prices_yes: dict[str, float], *, min_age_seconds: int = 3600) -> int:
        ...
        if self._use_supabase:
            ...
            for bet in open_bets:
                ...
                if agent:
                    new_balance = float(agent["balance"]) + stake + pnl
                    new_realized = float(agent["realized_pnl"]) + pnl
                    self._supabase.update(
                        AGENTS_TABLE,
                        {"balance": new_balance, "realized_pnl": new_realized, "updated_at": now},
        ...
        with self._connect() as conn:
            ...
            for bet in open_bets:
                ...
                conn.execute(
                    """
                    UPDATE bets
                    SET status = 'SETTLED', settled_at = ?, exit_price = ?, pnl = ?
                    WHERE id = ?
                    """,
    ...
    def leaderboard(self) -> list[dict[str, Any]]:
        ...
        with self._connect() as conn:
            rows = conn.execute(
                """
                SELECT name, balance, realized_pnl
                FROM agents
    ...
    def ticker(self, limit: int = 50) -> list[dict[str, Any]]:
        if self._use_supabase:
            rows = self._supabase.select(TICKER_TABLE, order="id.desc", limit=limit)
            return [dict(row) for row in rows]
        with self._connect() as conn:
            rows = conn.execute(
                """
    ...
    def resolve_agent_by_key(self, raw_key: str) -> str | None:
        ...
        with self._connect() as conn:
            row = conn.execute(
                "SELECT agent_name FROM agent_keys WHERE key_hash = ?",
                (key_hash,),
            ).fetchone()
        return str(row["agent_name"]) if row else None
    ...
    def export_state(self, *, markets: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        try:
            ...
            # Serverless file systems can be read-only/ephemeral; DB remains source of truth.
            pass
        return payload

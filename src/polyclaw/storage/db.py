...
from polyclaw.storage.schema import DASHBOARD_AGENT_ID, PRICE_TICKS_PARTITION_COUNT, metadata
...
def ensure_schema(engine: Engine) -> None:
    """Idempotent: bring any DB up to the current schema (Phase 1 + Phase 2a).

    Sequence (each step is a no-op if already applied):
      1. Phase 1 legacy backfill: add `agent_id` to trades/open_orders, rebuild the
         composite PK on paper_config, rebuild the surrogate PK on paper_positions.
      2. Phase 2a Postgres-only: create `price_ticks` as a partitioned-by-hash table
         up front, so `metadata.create_all` skips it on Postgres. SQLite sees a plain
         table via the regular create_all path.
      3. `metadata.create_all` fills in anything still missing (safe no-op otherwise).

    Running this twice is a no-op (Phase 1 mandatory test #1).
    """
    with engine.begin() as conn:
        _legacy_backfill_agent_id(conn)
        _ensure_new_pk_shapes(conn)
        _ensure_partitioned_price_ticks(conn)
    ...
    logger.info("Schema ensured on %s", engine.url.drivername)


def _ensure_partitioned_price_ticks(conn: Connection) -> None:
    """Create `price_ticks` as a hash-partitioned table on Postgres.

    SQLAlchemy's `Table` definition can't express partition clauses, so we intercept
    the table on Postgres and build it with raw DDL *before* `metadata.create_all`
    runs. `create_all(checkfirst=True)` will then see it already exists and skip.

    On SQLite this is a no-op — the regular `create_all` path handles the plain table.

    Idempotent: if the table is already present, we trust its shape and move on.
    """
    if conn.dialect.name != "postgresql":
        return
    if _table_exists(conn, "price_ticks"):
        return

    # Partitioned parent. The surrogate `id` can't be a simple PRIMARY KEY on a hash-
    # partitioned table in Postgres unless the partition key is part of it; we use
    # `GENERATED BY DEFAULT AS IDENTITY` and drop the PK constraint, since audit_log
    # references id purely for lookups and the index carries the uniqueness we need.
    ...
    # Per-partition indexes are created automatically via the parent index definition.
    conn.exec_driver_sql("CREATE INDEX idx_price_ticks_token_ts ON price_ticks (token_id, ts_ms)")
    conn.exec_driver_sql("CREATE INDEX idx_price_ticks_id ON price_ticks (id)")
    for i in range(PRICE_TICKS_PARTITION_COUNT):
        conn.exec_driver_sql(
            f"CREATE TABLE price_ticks_p{i} PARTITION OF price_ticks "
            f"FOR VALUES WITH (MODULUS {PRICE_TICKS_PARTITION_COUNT}, REMAINDER {i})"
        )
    logger.info("Created partitioned price_ticks with %d partitions", PRICE_TICKS_PARTITION_COUNT)
